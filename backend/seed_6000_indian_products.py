...
# Indian FMCG Brands & Product Categories
...
# Regional & Variant Modifiers to generate 6,000 authentic items
MODIFIERS = [
...
def generate_6000_products():
    db: Session = SessionLocal()
    try:
        print("Starting 6,000 Indian Product Seeding...")
        ...
        if needed <= 0:
            print(f"Database already contains {existing_count} products (>= 6000). Skipping generation.")
            return
        ...
        index = 0
        while len(products_to_add) < needed:
            ...
            # Price modeling based on category & unit size
            base_price = random.choice([5, 10, 20, 30, 40, 50, 65, 80, 99, 120, 150, 199, 240, 299, 350, 450, 550, 799, 999])
            if "10kg" in unit or "5L" in unit:
                base_price = random.choice([450, 550, 650, 799, 899, 1050, 1250])
            elif "5kg" in unit or "2L" in unit:
                base_price = random.choice([210, 250, 299, 340, 420])

            stock_level = random.randint(15, 200)
            low_stock = random.choice([5, 10, 15, 20])
            ...
            if len(products_to_add) % 1000 == 0:
                ...
                db.commit()
                print(f"Inserted {len(products_to_add)} / {needed} products...")
                products_to_add = []

        if products_to_add:
            db.add_all(products_to_add)
            db.commit()
            print(f"Inserted remaining products!")

        total = db.query(models.Product).count()
        print(f"🎉 SUCCESS! Total Products in Database: {total}")
